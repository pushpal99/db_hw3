...
export default function SongCard({ songId, handleClose }) {
  ...
  useEffect(() => {
    // First fetch the song data using songId
    fetch(`http://${config.server_host}:${config.server_port}/song/${songId}`)
      .then(res => res.json())
      .then(songResJson => {
        setSongData(songResJson);
        // Then, fetch the album data using album_id from the song data
        return fetch(`http://${config.server_host}:${config.server_port}/album/${songResJson.album_id}`);
      })
      .then(res => res.json())
