...
export default function HomePage() {
  ...
  const songColumns = [
    {
      ...
      renderCell: (row) => <Link onClick={() => setSelectedSongId(row.song_id)}>{row.song_title}</Link> // A Link component is used just for formatting purposes
    },
    {
      field: 'album_title',
      headerName: 'Album Title',
      renderCell: (row) => <NavLink to={`/albums/${row.album_id}`}>{row.album_title}</NavLink> // A NavLink component is used to create a link to the album page
    },
    {
      field: 'plays',
      headerName: 'Plays'
  ...
  const albumColumns = [
    ...
    },
    {
      field: 'total_plays',
      headerName: 'Plays',
      renderCell: (row) => <div>{row.total_plays}</div> // A NavLink component is used to create a link to the album page
    },
  ]
  ...
  return (
    <Container>
      ...
      <h2>Top Albums</h2>
      <LazyTable route={`http://${config.server_host}:${config.server_port}/top_albums`} defaultPageSize={5} rowsPerPageOptions={[5, 10]} columns={albumColumns} />
      <Divider />
